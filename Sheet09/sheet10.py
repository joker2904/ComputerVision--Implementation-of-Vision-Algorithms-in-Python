...
def eigen_map_to_bgr(img,eigen,step=10):
    h, w = img.shape
    y, x = np.mgrid[step/2:h:step, step/2:w:step].reshape(2,-1).astype(int)
    fx, fy = eigen[y,x].T
    vis = np.copy(img)
    cv.circle(vis, (fx,fy), 1, (0, 255, 0), -1)
    return vis
    pass
...
def flow_to_bgr(img,flow):
    ...
    angle *= (float)(90/3.141)
    scale = float(255.0)/(magnitude.max() -magnitude.min())
    mean_value = np.sum(magnitude)/(flow.shape[0]*flow.shape[1])
    polar = np.stack((magnitude,angle),axis=2)
    
    print(magnitude.shape,angle.shape,mean_value,polar.shape)
    '''
